...
p.prior <-   prior.p.dmc(
  p1=p.vector,
  dists=  rep("tnorm", length(p.vector)),
  p2= c(rep(1,1),1,rep(1,3),rep(1,4),rep(2,8)),
  lower=c(rep(0.1,1),0,rep(0,3),rep(0,4), rep(NA, 8)),
  upper=c(rep(1,1),rep(NA, length(p.vector)-1))
  )


dm <- data.model.dmc(dats,model)
top_samples_1t0 <- h.samples.dmc(nmc = 180,p.prior,dm)
save(top_samples_1t0, file="top_samples_1t0_LBA.RData")
...
dm <- data.model.dmc(dats,model)
sdvfixed_samples <- h.samples.dmc(nmc = 180,p.prior,dm)
save(sdvfixed_samples, file="sdvfixed_samples_LBA.RData")
...
p.prior <-   prior.p.dmc(
  p1=p.vector,
  dists=  rep("tnorm", length(p.vector)),
  p2= c(rep(1,1),1,rep(1,3),rep(1,4),rep(2,4)),
  lower=c(rep(0.1,1),0,rep(0,3),rep(0,4), rep(NA, 4)),
  upper=c(rep(1,1),rep(NA, length(p.vector)-1))
  )


dm <- data.model.dmc(dats,model)
meanvfixed_samples <- h.samples.dmc(nmc = 180,p.prior,dm)
save(meanvfixed_samples , file="meanvfixed_samples_LBA.RData")
...
p.prior <-   prior.p.dmc(
  p1=p.vector,
  dists=  rep("tnorm", length(p.vector)),
  p2= c(rep(1,1),1,rep(1,3),rep(1,2),rep(2,8)),
  lower=c(rep(0.1,1),0,rep(0,3),rep(0,2), rep(NA, 8)),
  upper=c(rep(1,1),rep(NA, length(p.vector)-1))
  )
